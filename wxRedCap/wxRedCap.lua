...
function p_init()
    ...
    if (w+h) == (640+1136) then
        -- iphone 5s
        _hr_x, _hr_y, _hr_c = 135, 900, 0xfa9d3b;
...
function handle_hongbao_received()
	keepScreen(true);
	-- local x_img, y_img = findImage("red.png", 0, 0, _screen_w, _screen_h, 10050000);
	local x_img, y_img = findMultiColorInRegionFuzzy(0xe78538,
        "0|-34|0xf04c2d,0|45|0xda421f,-34|0|0xd83c1e,33|0|0xda401f",
        95, 0, 0, _screen_w, _screen_h);
	keepScreen(false);
	-- _xprintf("image %d, %d", x_img, y_img);
	if x_img ~= -1 and x_img ~= -1 then
		_hr_x = x_img+30;
		_hr_y = y_img+30;
